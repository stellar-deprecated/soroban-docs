...
This can be a bit tricky to wrap your head around, but the conventions make sense once you let it sink in. The examples below are provided only in Python, for the sake of brevity.

In the previous examples, the `COUNTER` _key_ was used as a `LedgerKey` while the incremented _value_ was stored in a **`LedgerEntry`**. "Ledger Entry" is the relevant term to keep in mind during this discussion. That `LedgerEntry` was stored on the Stellar ledger, and was associated with a corresponding `LedgerKey`. `LedgerKey: LedgerEntry` works the same way you would think of almost any `key: value` storage system. In addition to whatever `ContractData` `LedgerEntry`s a contract may (or may not) store on the ledger, there is also a `ContractCode` entry that identifies the relevant WASM byte-code. We retrieve that byte-code by _first_ retrieving the `LedgerKey` associated with it.
...
And the response we get contains (even more) `LedgerEntryData` that we can decode and parse to get the actual, deployed, real-life contract byte-code. We'll leave that exercise up to you.
