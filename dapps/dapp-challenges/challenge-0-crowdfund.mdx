...
This challenge will guide you through the process of building and shipping a crowdfunding dapp on Stellar using Soroban. You will learn how to deploy smart contracts to Testnet, and how to interact with them through a web frontend. In this context, the term "ship" refers to finalizing the development process of your dapp, ensuring that it functions as expected, and is accessible for user interaction and testing through a hosted frontend. However, it's crucial to clarify that despite its functionality, the dapp is not promoted nor intended for deployment in a production-level setting on Testnet. The challenge is designed for educational purposes, helping you understand how a dapp can be built and interacted with, with further customization and development, it has the potential to evolve into a full-fledged, ready-to-use crowdfunding solution.
...
> Note: These are test lumens for use with your local network and cannot be used on Testnet or Mainnet
...
> Note: These are test tokens for use with your local network and cannot be used on Testnet or Mainnet
...
Now that your dapp is fully functional locally, next prepare it for deployment on Testnet.
...
Then, run the following commands to initialize the contracts on Testnet:
...
Ensure that you are connected to Testnet and you should be able to see the frontend of your dapp!
...
🌐 Explore [Stellar Laboratory] to inspect your account assets on Testnet.
