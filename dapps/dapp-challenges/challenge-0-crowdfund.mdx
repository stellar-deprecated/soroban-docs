...
Soroban CLI is the command line interface to Soroban. It allows you to build, deploy, and interact with smart contracts; configure identities; generate key pairs; manage networks; and more. The soroban-cli alias that is used in this challenge...

## Checkpoint 1: 🎬 Deploy Smart Contracts

Now that you have the Crowdfund branch checked out, it's time to deploy the smart contracts to a Sandbox environment. Deploying a smart contract typically involves submitting the contract code to the blockchain network, where it becomes part of the chain's immutable ledger. Deploying smart contracts to a Sandbox environment simulates that process without actually affecting the blockain. When you "ship" the dapp later on, you'll instead deploy to Futurenet.
...
Now that you have the frontend running, it's time to connect it with the backend, your smart contract, that defines the rules and logic of the crowdfund campaign, including the function for accepting contributions. If you want to dig into the specifics of the contract, take a look at...
...
Fuel the vision! In this step, you will learn how to mint tokens and fund the crowdfunding campaign. Minting tokens in a crowdfund dapp, while not always required, serves an important purpose: ... 
...
Now that your dapp is fully functional, you'll prepare it for a new deployment on Futurenet, this time with a hosted frontend. To do this, you'll reset the dapp that you originally deployed in the Sandbox environment, effectively readying it for new users to mint and pledge funds to your campaign.

To initialize the contract on Futurenet, run the following commands:
...
Next, you will need to remove unneccessary files for the the deployment. These unnecessary files include... Run the following command to remove the `target` directory:

```sh
rm -rf target
```

Next, you will use the Vercel cli to complete your deployment. Vercel offers a quick and effective way to easily create a frontend...
