...
In the context of the example Crowdfunding dapp, understanding how to build Docker images is crucial. The Docker images serves as the basis for our container, which provide the environment for our dapp to run.
...
These are the main components that make up the Crowdfunding Dapp that we'll be exploring in the next sections.
...
The `Pledge` component is where users interact with the Crowdfunding Dapp, pledging tokens to the campaign of their choice. It serves as a data entry point, data fetcher and presenter.
