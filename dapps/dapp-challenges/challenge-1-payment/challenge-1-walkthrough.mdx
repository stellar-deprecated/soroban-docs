---
sidebar_position: 1
title: Example Soroban Payment Dapp Walkthrough
description: Step through the code behind the Example Soroban Payment Dapp.
---

# Creating a Payment Dapp with Soroban

We'll be exploring an example [Soroban Payment Dapp](https://github.com/stellar/soroban-react-payment) built by the Wallet Engineering team at SDF. The Dapp is built using [React](https://reactjs.org/) and [TypeScript](https://www.typescriptlang.org/). It uses the [Freighter](https://www.freighter.app/) wallet to connect to the Futurenet and sign transactions.
...
The Example Soroban Payment Dapp relies on the following dependencies:
...
To run the example contract and set up a Payments Dapp, follow the steps below:
...
The `SendPayment` component is a part of a Soroban Payment Frontend and is responsible for handling the creation of a payment. Let's delve into the details of this component:
...
- If there is any error during these steps, it logs the error, and sets the error message to "Unable to fetch token details."
...
- `Decimal Precision`: A precision value of 7 is specified, indicating that the token supports transactions with up to 7 decimal places. This level of precision allows for fine-grained control and flexibility when transferring token amounts.
...
Lastly, you need to mint some tokens to the sender's account. Run the following command, replacing` <TOKEN_CONTRACT_ID>`, `<ADMIN_ACCOUNT_SECRET_KEY>`, and `<USER_PUBLIC_KEY> `with the corresponding values:
...
Now that we've dived into the Payment Dapp, let's take on the [Payment Dapp challenge](./challenge-1-payment.mdx)!
