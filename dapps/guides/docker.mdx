...
To streamline the deployment process, we use a `quickstart.sh` script. This script is a wrapper for the `stellar/quickstart` Docker image, which provides a quick way to run a Stellar network. You can find an example of the `quickstart.sh` script located in the root directory of the Crowdfund Dapp.
