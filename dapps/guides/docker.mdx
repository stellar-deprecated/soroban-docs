...
In the context of the example Soroban dapps, understanding how to build Docker images is crucial. The Docker images serves as the basis for our container, which provide the environment for our dapp to run.

Heres an example from our example

To illustrate the process, let's take an example from our [Example Crowdfund Dapp](https://github.com/stellar/soroban-example-dapp/tree/main). In order to build the Docker image, we utilize a command that is encapsulated within our Makefile:
