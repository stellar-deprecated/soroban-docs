...
## Dapp Demos
For easy accessibility, we have compiled the dapps into a single repository, [Scaffold Soroban](https://github.com/stellar/scaffold-soroban), which contains the following dapps:
...
Other utilities such as `accountToScVal` and `numberToI128` are also provided to simplify transaction creation by converting user-friendly inputs into the formats expected by the Soroban RPC on the Stellar network. The dapps are built with premade helper functions such as the `makePayment` function which facilitates streamlined "transfer" operations and also includes memos for supplementary transaction-related information.
