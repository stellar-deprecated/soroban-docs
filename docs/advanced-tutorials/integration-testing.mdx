...
When combining native imports with wasm imports in your integration testing, be cautious. This mix can lead to type redefinitions that are incompatible, potentially causing unforeseen challenges in your test environment
...
A solution to maintain type-checking and compatibility is to implement shared structs in a third-party crate that can be referenced by both contracts. However, integrating multi-contracts remains a challenge in Soroban, and developers are advised to keep an eye on official documentation and examples for updates and best practices.
