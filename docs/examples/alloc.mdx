...
[contract rust dialect]: ../learn/rust-dialect.mdx
...
:::info The heap memory in the context of a smart contract actually refers to the WASM linear memory. The `alloc` will use the global allocator provided by the soroban sdk to interact with the linear memory. :::

:::caution Using heap allocated array is typically slow and computationally expensive. Try to avoid it and instead use a fixed-sized array or `soroban_sdk::vec!` whenever possible.
