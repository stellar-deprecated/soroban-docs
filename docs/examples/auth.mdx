...
contract needs to start storing other types of data for `Address`es, it can do
so by adding additional variants to the enum.
:::
...
because it is the first increment for the user.
...
for the user. The first users data is separate to that of the second users.
