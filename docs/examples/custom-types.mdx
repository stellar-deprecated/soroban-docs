...
[oigp]: https://gitpod.io/#https://github.com/stellar/soroban-examples/tree/v0.3.2

[custom types example]: https://github.com/stellar/soroban-examples/tree/v0.3.2/custom_types
[storing data example]: storing-data.mdx

## Run the Example

First go through the [Setup] process to get your development environment
configured, then clone the `v0.3.2` tag of `soroban-examples` repository:

[Setup]: ../getting-started/setup.mdx

```
git clone -b v0.3.2 https://github.com/stellar/soroban-examples
```
...
Ref: https://github.com/stellar/soroban-examples/tree/v0.3.2/custom_types
...
Enums containing unit and tuple variants are stored on ledger as a two
...
Only unit variants and single value variants, like `A` and `B`
below, are supported.
...
Types that have been annotated with `#[contracttype]` can be stored as contract
...
The test invokes the `increment` function on the registered contract that causes
the `State` type to be stored and updated a couple times.
...
The test then invokes the `get_state` function to get the `State` value that was
stored, and can assert on its values.
...
Run it a few more times with different increment amounts to watch the count
change.

Use the `soroban` to inspect what the counter is after a few runs.

```sh
soroban read --id 1 --key STATE
```
