...
:::info In this example there are two contracts that are compiled separately, and the tests deploy one with the other. :::
...
See the [tests](#tests) for an example of installing the contract code in tests. For the actual on-chain installation see the general deployment [tutorial](https://soroban.stellar.org/docs/tutorials/deploy-to-futurenet).
