...
single function that takes one input and returns it as an output.  To learn more
about how a Soroban smart contract works, check out this link for a 
[High Level Overview]

[High Level Overview]: https://soroban.stellar.org/docs/learn/high-level-overview
...
also be used. Floats and floating point math are not supported.

Contract inputs must not be references.  They must be passed by xxxx by using the
& prefix.
...
in the argument list for most functions.  You can learn more about how the Env
functions here [Soroban Env]

[Soroban Env]: https://soroban.stellar.org/docs/learn/environment-concepts
...
Soroban environment that the contract will run in.  Here we create a default
environment.  Addition arguments can be passed to customize it like this...

```rust
let env = Env::default();
```

The contract is registered with the environment using the contract type.  The
first argument is the xxx, and the second argument is the contract name, the 
same as in the lib.rs file, impl HelloContract
...
using it.  The parameters supplied to the invoke are
--wasm This is the code for the smart contract to be run on the blockchain env
--id  This is the smart contract id, in this case 1 because (explain why)
--fn This is the function inside the smart contract that is being called.
--arg This is the first argument passed into the hello function

From a overview perspective, the CLI is basically calling HelloContract.hello('friend');
