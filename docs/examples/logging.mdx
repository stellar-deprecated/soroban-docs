...
[logging example]: https://github.com/stellar/soroban-examples/tree/v0.4.2/hello_world

:::tip Logs are no a substitute for step-through debugging. Rust tests for Soroban can be step-through debugged in your Rust-enabled IDE. See [testing] for more details. :::

:::caution Logs are not accessible by dapps and other applications. See the [events example] for how to produce structured events. :::
...
Ref: https://github.com/stellar/soroban-examples/tree/v0.4.2/logging
...
:::caution The values outputted are currently relatively limited. While primitive values like `u32`, `u64`, `bool`, and `Symbol`s will render clearly in the log output, `Bytes`, `Vec`, `Map`, and custom types will render only their handle number. Logging capabilities are in early development. :::
