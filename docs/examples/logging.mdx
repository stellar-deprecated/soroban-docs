...
the [`soroban`] CLI. Logs are only compiled into the contract if the
`debug-assertions` Rust compiler option is enabled.

[logging example]: https://github.com/stellar/soroban-examples/tree/v0.0.4/hello_world
...
[testing]: ../tutorials/testing.mdx
[events example]: events.mdx
[`soroban`]: ../getting-started/setup#install-the-soroban-cli
...
```
Ref: https://github.com/stellar/soroban-examples/tree/v0.0.4/logging

## How it Works

The [`log!`] macro logs a string. Any logs that occur during execution are
outputted to stdout in the [`soroban`] CLI and available for tests to assert on
or print.

Logs are only outputted if the contract is built with the `debug-assertions`
compiler option is enabled. This makes them efficient to include in code since a
regular `release` build will omit them.
...
To enable the `debug-assertions` compiler option a new `release-with-logs`
profile is added to `Cargo.toml` that inherits from the `release` profile.
...
To build for deployment use the `--release` or `--profile release` option.
...
more complex types like `Bytes`, `Vec`, `Map`, or custom types will render only
their handle identifier. The capabilities of logging is in development.
:::
