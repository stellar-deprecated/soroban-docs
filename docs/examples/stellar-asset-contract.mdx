...
The SAC implements the [token interface](./token-example-and-interface.mdx), which is
...
[here](./invoking-contracts-with-transactions). The resulting token
...
See the [advanced auth example](./auth-advanced.mdx) for an overview of
authorization.
...
the [invoker](./auth.mdx#invoker) auth. In this way there won't be
...
[advanced auth](./auth-advanced.mdx). The following snippet shows how
...
extends the common [token interface](./token-example-and-interface.mdx).
