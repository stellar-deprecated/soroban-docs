...
[increment example]: https://github.com/stellar/soroban-examples/tree/v0.3.2/increment
...
```
Ref: https://github.com/stellar/soroban-examples/tree/v0.3.2/increment
...
Contract data is associated with a key. The key is the
...
`symbol!(...)` they are computed at compile time and stored in code as a 64-bit
...
Rerun the invoke with the `--footprint` option to view the [footprint] of the invocation,
which is the ledger entries that the contract will have read or written to.
...
:::info
Soroban is a pre-release and at this time outputs footprints in an unstable JSON
format.
:::
