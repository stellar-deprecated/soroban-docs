...
- Ledger space rent: The payment for the ledger entry rent bumps and rent payments for increasing ledger entry size. Refer to [state expiration](./state-expiration.mdx) section for more details on how the rent works.
...
Rent payment is a write fee for entry payed per every 'rent period' (10 months for temporary entries, 1 month for persistent entries).
...
The limits may change in the mainnet release. They can also be updated, usually increased, based on the network validator consensus.
