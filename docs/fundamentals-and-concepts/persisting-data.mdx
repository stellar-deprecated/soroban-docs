...
- Prefer `Temporary` over `Persistent` and `Instance` storage
  - Anything that can have a timeout should be `Temporary` with TTL set to the timeout. See the [resource limits table](../fundamentals-and-concepts/fees-and-metering.mdx#resource-limits) for the current maximum TTL/timeout.
