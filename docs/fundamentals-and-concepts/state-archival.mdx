...
All contract data has a Time To Live (TTL) that must be periodically extended. If an entry's TTL is not periodically extended, the entry's TTL will eventually go to 0 and become "archived". Each type of storage functions similarly, but has different fees and archival behavior:
...
An entry's TTL is defined as how many ledgers remain until the entry is no longer live.
...
network parameter and defaults to 1 year's worth of ledgers. This maximum TTL is not enforced
