...
function submitOrRestoreAndRetry(
  ...
  return s
    ...
    .then((sim: SorobanRpc.SimulateTransactionResponse) => {
      if (SorobanRpc.isSimulationError(resp)) {
        throw resp;
      }

      // If simulation didn't fail, we don't need to restore anything!
      if (SorobanRpc.isSimulationSuccess(resp) && !resp.restorePreamble) {
      ...
      //
      return s.getAccount(signer.publicKey()).then((account) => {
        const restoreFee = parseInt(BASE_FEE);
        restoreFee += parseInt(resp.restorePreamble.minResourceFee);

        const restoreTx = new TransactionBuilder(account, {
          fee: restoreFee.toString(),
          networkPassphrase: Networks.FUTURENET,
          sorobanData: resp.restorePreamble.transactionData.build(),
        })
          .addOperation(Operation.restoreFootprint({}))
          .build();

        restoreTx.sign(signer);
        return yeetTx(restoreTx);
      });
    })
