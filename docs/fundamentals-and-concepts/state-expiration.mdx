...
After that, we can (simulate and) run our initial transaction again.

Let's see that in code. Here's our
...
function submitOrRestoreAndRetry(
  signer: Keypair,
  tx: Transaction,
  sim: SorobanRpc.SimulateTransactionResponse,
): Promise<SorobanRpc.GetTransactionResponse> {
  return yeetTx(tx)
...
function yeetRestoreTx(
  ...
  return s
    ...
    .then((account) => {
      ...
      return { tx, simData: s.simulateTransaction(tx, tx.networkPassphrase) };
    })
    .then(({ tx, simData }) => {
      //
      ...
      const preppedTx = TransactionBuilder.cloneFrom(tx)
        .setSorobanData(readSorobanData(simData.transactionData))
        .setFootprint([], ledgerKeys)
        .build();
...
Notice an important quirk in the above code: We don't know which entries are the expired ones, so we will blindly try restoring all of them (by taking the union of the read-only and read-write keys). We _could_ find out which ones expired by calling [`getLedgerEntries()`](https://soroban.stellar.org/api/methods/getLedgerEntries), then parsing each resulting entry to find its `expirationLedgerSeq`, but this is an extra round-trip:

```typescript
const expiredLedgerKeys = await s
  .getLedgerKeys(ledgerKeys)
  .then((resp: GetLedgerEntriesResponse) =>
    ledgerKeys.filter((ledgerKey, idx) => {
      const rawEntry = resp.entries[idx];
      const entry = xdr.LedgerEntryData.fromXDR(entry.xdr, "base64");
...
As you can imagine, if the ledger cannot find your deployed contract instance or the code that backs it, it can't load it to execute your invocations.
...
Let's work through how these can be recovered. They are slightly different for a convenient reason: we don't need simulation to figure out the footprints, though you still can do it that way. Instead, we can leverage `Contract.getFootprint()`, which prepares a footprint:
