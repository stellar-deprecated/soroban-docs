...
1. First, we simulate our transaction as we normally would.
2. Then, we perform restoration via [`Operation.restoreFootprint`](https://stellar.github.io/js-soroban-client/Operation.html#.restoreFootprint) if the simulation told us that we need to.
3. Finally, we can actually run our initial transaction.
...
function submitOrRestoreAndRetry(
  ...
  if (!SorobanRpc.isSimulationSuccess(resp)) {
    throw resp;
  }

  // If simulation didn't fail, we don't need to restore anything!
  if (!resp.restorePreamble) {
  ...
  const fee = parseInt(BASE_FEE);
  fee += parseInt(resp.restorePreamble.minResourceFee);

  const restoreTx = new TransactionBuilder(account, { fee: fee.toString() })
    .setNetworkPassphrase(Networks.FUTURENET),
    .setSorobanData(resp.restorePreamble.transactionData.build())
  ...
  // up-to-date)
  //
  const retryTxBuilder = TransactionBuilder.cloneFrom(tx).setSorobanData(
    sim.transactionData,
  );
...
Notice something in the above code: When restoration is required, **simulation still succeeds**. The way that we know that something needs to be restored is the presence of a `restorePreamble` in the RPC's response. This structure contains both the resource/footprint data and fee needed for restoration, and the rest of the response contains the simulation result **as if** that restoration was done first. This means fewer round-trips to get going again!
...
function restoreContract(
  ...
  return s
    ...
    .then((account) => {
      ...
      return s.prepareTransaction(restoreTx, Networks.FUTURENET);
    })
    .then((prepTx) => {
      prepTx.sign(signer);
      return yeetTx(prepTx);
