...
function submitOrRestore(
  ...
  return yeetTx(tx).then((resp: SorobanRpc.GetTransactionResponse) => {
    ...
    //
    // Important: blindly restoring all keys.
    //
    const ledgerKeys = storageUsage.readOnly().concat(storageUsage.readWrite());

    return buildRestoreTx(signer, ledgerKeys)
      .then((restoreTx) => s.simulateTransaction(restoreTx, Networks.FUTURENET))
      .then((simTx) => {
        if (simTx.error || (simTx.results?.length ?? 0) !== 1) {
          throw simTx;
        }

        const simData = readSorobanData(sim.transactionData);
...
async function buildRestoreTx(signer: Keypair, ...ledgerKeys: xdr.LedgerKey[]) {
  return s
    ...
    .then((account) => {
      ...
      return s.prepareTransaction(tx, tx.networkPassphrase);
    })
    .then((preppedTx) => {
      preppedTx.sign(signer);
      return awaitTransaction(preppedTx);
    });
}
```
