...
```typescript
import { Transaction, SorobanRpc } from "soroban-client";
...
function submitOrRestore(
  ...
  return yeetTx(tx).then((resp: SorobanRpc.GetTransactionResponse) => {
    ...
    const restoreTx = buildRestoreOp(signer, ledgerKeys);

    return s
      .simulateTransaction(restoreTx, Networks.FUTURENET)
      .then((simTx) => {
        if (simTx.error || (simTx.results?.length ?? 0) !== 1) {
          throw simTx;
        }

        const simData = xdr.SorobanTransactionData.fromXDR(
          sim.transactionData,
          "base64",
        );
        ...
        simData.resources().footprint().readOnly();
        simData.resources().footprint().readWrite(ledgerKeys);
