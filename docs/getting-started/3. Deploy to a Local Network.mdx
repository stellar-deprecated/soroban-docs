...
:::info It's also possible to run a contract in the local sandbox environment without a network using just the [`soroban-cli`]. See [Run on Sandbox] for more details. :::
...
:::tip It's a good practice to never use the same keys for testing and development that you use do for the public Stellar network. Generate new keys for testing and development and avoid ever using them for other purposes. :::
...
:::tip It's good practice to avoid entering secret keys on the command line. You can also specify the secret key by setting the environment variable `SOROBAN_SECRET_KEY`. The [`soroban-cli`] is still in development and in the future there will be other secure ways to provide a secret key, such as via hardware keys. :::
...
[Write a Contract]: 1.-hello-world#write-a-contract
[Run on a Local Network]: run-on-local-network
[Build]: 1.-hello-world#build
...
[Run on Sandbox]: 1.-hello-world#run-on-sandbox
