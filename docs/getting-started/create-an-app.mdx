...
You could build a Soroban app with any frontend toolchain, or integrate it into any existing full-stack app. For this tutorial, we're going to use [Astro](https://astro.build/). Astro works with React, Vue, Svelte, or any other UI library that you might like, including none, which is what we'll use here. The Soroban-specific parts of this tutorial will be similar no matter what frontend toolchain you use.

If you're new to frontend, don't worry. We won't go too deep. But it will be useful for you to see and experience the frontend development process used by Soroban apps. We'll cover the bits of JavaScript and Astro that are relevant, but teaching all of frontend development and Astro is beyond the scope of this tutorial.

Let's get started.

You're going to need [Node.js](https://nodejs.org/en/download/package-manager/). If you haven't yet, install it now.
...
soroban contract bindings typescript \
  ...
  --contract-name hello-soroban \
  --output-dir hello-soroban-client
```

:::tip Why --wasm?
You may have noticed that we gave Soroban everything it needed to create this library _twice_. Given the network info and contract ID, it can already fetch the Wasm. But we also gave it the path to the Wasm file. Why?

This is a temporary limitation of Soroban CLI. In an upcoming version, you will be able to omit the `--wasm` option, if using a contract from the network.

So why not _only_ provide `--wasm`, and leave off the network stuff? Because `contract bindings typescript` makes use of the network settings within the generated library code, to make it easy to interact with that specific deployed contract.

Today, since you need both, how do you work with someone else's contract? You could first use `soroban contract fetch` to save that contract's Wasm to a local file, then use a command like the one above. Learn more with `soroban contract fetch --help`.
:::
...
-   "astro": "astro"
+   "astro": "astro",
+   "postinstall": "soroban contract bindings typescript --wasm target/wasm32-unknown-unknown/release/hello_soroban.wasm --network futurenet --contract-id $(cat .soroban/hello-id) --contract-name hello-soroban --output-dir node_modules/hello-soroban-client"
  }
```
...
+
+const greeting = await hello({ to: 'Soroban' });
 ---
```
...
<script>
  ...
  async function setLoggedIn() {
    ...
    wrap.innerHTML = `Signed in as ${publicKey}`;
    wrap.title = publicKey;
  }
</script>
```

Some of this may look surprising. `<style>` and `<script>` tags in the middle of the page? Styles on a bare `div`? `import` statements in a `<script>`? Top-level `await`? What's going on here?

Astro automatically scopes the styles within a component to that component, so there's no reason for us to come up with a clever name for the `class` of this `div`.
...
The code itself here is pretty self-explanatory. We import a few methods from `@stellar/freighter-api` to check if the user is logged in. If they already are, then `isAllowed` returns `true`, and we replace the contents of the `div` with the signed-in message, replacing the button. If not, we add a click handler to the button to prompt the user to connect Freighter with `setAllowed`. Once they do, we again replace the contents of the `div` with the signed-in message. The [`aria` stuff](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Live_Regions) ensures that screen readers will read the new contents when they're updated.
...
We're going to a contract client for the incrementor contract with a similar command to the one we used before. Let's add it directly to the `postinstall` script:

```json
"postinstall": "soroban contract bindings typescript --wasm target/wasm32-unknown-unknown/release/hello_soroban.wasm --network futurenet --contract-id $(cat .soroban/hello-id) --contract-name hello-soroban --output-dir node_modules/hello-soroban-client && soroban contract bindings typescript --wasm target/wasm32-unknown-unknown/release/incrementor.wasm --network futurenet --contract-id $(cat .soroban/incrementor-id) --contract-name incrementor --output-dir node_modules/incrementor-client"
```
...
Also notice that you don't need to manually specify Freighter as the wallet in the call to `increment`. This may change in the future, but while Freighter is the only game in town, these generated libraries automatically use it.
...
Check the page; if you're still running your dev server, it should have already udpated. Click the "Increment" button; you should see a Freighter confirmation. Confirm, and... the value updates! 🎉
...
Next, let's add a `setup` script that creates and funds the `alice` identity, then builds and deploys your contracts:

```json
"scripts": {
  ...
  "setup": "cargo build && soroban config identity generate alice && curl \"https://friendbot-futurenet.stellar.org/?addr=$(soroban config identity address alice)\" && soroban contract deploy --wasm target/wasm32-unknown-unknown/release/hello_soroban.wasm --source alice --network futurenet > .soroban/hello-id && soroban contract deploy --wasm target/wasm32-unknown-unknown/release/incrementor.wasm --source alice --network futurenet > .soroban/incrementor-id && npm install",
  ...
}
```
...
- [Deploy](https://docs.astro.build/en/guides/deploy/) your frontend. You can do this quickly and for free [with GitHub](https://docs.astro.build/en/guides/deploy/github/).
