...
We're going to a contract client for the incrementor contract with a similar command to the one we used before. Let's add it directly to the `postinstall` script, using a double ampersand ([&&](https://stackoverflow.com/a/25669618/249801)) to run both `bindings typescript` commands in a row:
...
Next, let's add a `setup` script that creates and funds the `alice` identity, then builds and deploys your contracts. This is a lot for a single NPM script; look for the `&&` to see where one command ends and the next begins. Add the following to the `scripts` section of `package.json`:

```json
"scripts": {
  ...
  "setup": "soroban contract build && soroban config identity generate alice && curl \"https://friendbot-futurenet.stellar.org/?addr=$(soroban config identity address alice)\" && soroban contract deploy --wasm target/wasm32-unknown-unknown/release/hello_soroban.wasm --source alice --network futurenet > .soroban/hello-id && soroban contract deploy --wasm target/wasm32-unknown-unknown/release/incrementor.wasm --source alice --network futurenet > .soroban/incrementor-id && npm install",
  ...
}
```
