...
The template included several scripts in the root `package.json` file, including a `postinstall` script that calls `bindings:hello` after every install to make sure that the generated library stays up-to-date.

Let's run `npm install` now to make sure that all of our dependencies are installed, and note that the `postinstall` script will also run. This means you won't need to manually run the `soroban contract bindings typescript` command again.
...
To call the `hello` method on our `hello-soroban` contract, run the following with the `to` argument set to "RPC". This again makes use of command expansion `$(…)`; see the note about that in the [Configure an Identity](setup#configure-an-identity) section of Setup.
