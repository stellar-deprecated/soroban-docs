...
You will also notice here (and with any other contract run in the sandbox environment) some diagnostic output will be displayed on the screen. This is because `soroban-cli` automatically sets the diagnosticlevel to "Debug" for contracts running in the sandbox.
