...
:::tip Use the `release` profile outlined in [Configure the Release Profile], otherwise the contract will exceed Soroban's size limits. :::
...
:::tip To further optimize builds to be as small as possible, see [Optimizing Builds]. :::
...
:::info It's also possible to run a contract on a fully featured local network. See [Deploy to a Local Network] for more details. :::
...
:::tip Building optimized contracts is only necessary when deploying to a network with fees or when analyzing and profiling a contract to get it as small as possible. If you're just starting out writing a contract, these steps are not necessary. See [Build] for details on how to build for development. :::
