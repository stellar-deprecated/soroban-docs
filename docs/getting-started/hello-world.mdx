...
### Contracts
The `contracts` directory is where our Soroban contracts live. Each contract will have it's own sub-directory with source code, test code and a `Cargo.toml` file. 
...
The boiler plate Astro web app files live in the `src` directory. This includes  the Astro components, layouts, and pages. We'll look at these pieces a bit closer later in this tutorial.
...
#### Configuring the `release-with-logs` Profile
...
See the [logging example](../basic-tutorials/logging.mdx) for more information about how to log.
