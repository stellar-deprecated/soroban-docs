...
Building optimized contracts to be as small as possible requires some additional tools and Rust toolchain.
...
### Build with optimization flags and `soroban contract optimize`

The additional options instruct the compiler to remove unnecessary information from the contract.
