...
The `.wasm` file contains the logic of the contract, as well as the contract's [specification / interface types](../soroban-internals/fully-typed-contracts.mdx), which can be imported into other contracts who wish to call it. This is the only artifact needed to deploy the contract, share the interface with others, or integration test against the contract.
