...
The Soroban CLI can connect with and work with multiple networks. Here we will highlight two of those networks: The SDF-operated "Testnet," and a self-deployed "Local" network that runs on a local machine through a [Docker](https://www.docker.com/) quickstart image.

It can be tricky to choose between these options, so here are some brief considerations you may want to think about.

**A Local test network...**

- Requires Docker to operate on a single, local machine.
- Closes a new ledger every one (1) second. This makes it faster to develop against.
- Allows for customizing resource limits. You can start developing your contract with "unlimited" resources, and then tune it to meet the Testnet/Mainnet requirements.
- Can facilitate reproducible test runs with a blank slate.
- Is still available if the Testnet network is congested.
- Is not affected by Testnet resets.

** The Testnet test network...**

- Does not require Docker or any other software to start. It is already running, around the clock.
- Is publicly and globally available, so your development work can easily be shared with others.
- Is completely reset about once per quarter, erasing all data.
- Mimics the software setup and constraints on Mainnet. This includes resource limits, ledger close times, etc.
...
The Local network is a fully featured Stellar network that runs the same protocol, software, and services as the Testnet, Futurenet, or even Mainnet networks. The main difference is that it's isolated to your own local machine. You can also benefit from one-second ledger close times, making it a faster network to develop against.
