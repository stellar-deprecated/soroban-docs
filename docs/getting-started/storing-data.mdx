...
This tutorial assumes that you've already completed the previous steps in Getting Started: [Setup](setup.mdx), [Hello World](hello-world.mdx), [Deploy to Testnet](deploy-to-testnet.mdx), and [Multi-Contract Projects](multi-contract-projects.mdx).
...
entry's lifetime is not periodically bumped, the entry will eventually reach the end of its lifetime and "expire". You can learn more about this in the [State Expiration](../soroban-internals/state-expiration.mdx) document.
