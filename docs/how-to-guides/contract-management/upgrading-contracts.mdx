...
To build the contract `.wasm` files, run `cargo build --target wasm32-unknown-unknown --release` in both `upgradeable_contract/old_contract` and `upgradeable_contract/new_contract` in that order.
