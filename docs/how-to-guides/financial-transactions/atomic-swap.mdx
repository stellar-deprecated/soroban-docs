...
This is example demonstrates advanced usage of Soroban auth framework and assumes the reader is familiar with the [auth example](../development-basics/auth.mdx) and with Soroban token usage.
...
`env.auths()` returns all the authorizations. In the case of `swap` four authorizations are expected. Two for each address authorizing, because each address authorizes not only the swap, but the `increase_allowance` all on the token being sent.
