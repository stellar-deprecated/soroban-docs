...
pub trait VaultTrait {
    ...
    // sends `amount` of vault share tokens to this contract, burns all vaults share tokens
    // in this contract, and sends the
...
We should receive a SUCCESS message with the token contract ID. We will need this ID for the next step.
...
We should receive a SUCCESS message with the vault contract ID. We will need this ID for the next step.
...
It's important to note that the vault contract id will need to change formats for the call to work. We can convert the vault contract id to the correct format by pasting its address in a [Hex to StrKey Tool](https://strkey-encode.xycloo.com/).
...
After receiving a SUCCESS message, we can check the balance of the account. We can do this by running the `balance.sh` script.
...
We should receive a SUCCESS message with the balance of the account.
...
After receiving a SUCCESS message, we can deposit some tokens into our vault. We can do this by running the `deposit.sh` script.
...
After receiving a SUCCESS message, we can check the reserves of the vault. We can do this by running the `reserves.sh` script.
...
We should receive a SUCCESS message with the reserves of the vault.
...
We should receive a SUCCESS message with the withdrawal amount.
...
It's important to note that this is not a production-ready contract and is only meant to demonstrate the capabilities of the Soroban smart contract platform. We hope to see much more complex yield contracts deployed on the Soroban in the future, and we hope you will be a part of it!
