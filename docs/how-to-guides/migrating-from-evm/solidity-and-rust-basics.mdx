...
This is a Rust attribute that tells the Rust compiler not to link the [Rust standard library](https://doc.rust-lang.org/std/). Since the Soroban Rust SDK uses its own set of logic, it doesn't need to utilize the Rust standard library, so this attribute is needed.
