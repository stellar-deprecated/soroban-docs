...
The [Soroban Rust SDK](https://docs.rs/soroban-sdk/0.7.0/soroban_sdk/index.html) supports a variety of primitive and [built-in data types](../../learn/built-in-types), such as:
...
- Bool (`bool`)
- Symbols (`Symbol::new`): small efficient strings up to 32 characters in length and (`Symbol::short`) strings up to 9 characters in length, both are limited to the characters `a-zA-Z0-9_` and are encoded into 64-bit integers
...
let symbol_new = Symbol::new("SampleSymbolExpression"); // up to 32 chars
