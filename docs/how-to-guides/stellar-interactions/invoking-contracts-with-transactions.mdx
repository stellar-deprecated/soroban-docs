...
Every Soroban transaction has to have a `SorobanTransactionData` transaction [extension] populated. This is needed to compute the [Soroban resource fee](../learn/fees-and-metering.mdx).
...
The simplest method to determine the values in `SorobanResources` is to use the transaction simulation with [preflight service](../learn/interacting-with-contracts.mdx#preflight).
