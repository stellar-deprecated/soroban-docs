...
See the [authorization overview](../learn/authorization.mdx) and [auth example](./auth.mdx) for general information about authorization in Soroban.
...
- [Timelock](timelock.mdx) and [single offer](single-offer-sale.mdx) move token via `xfer` to and from the contract
- [Atomic swap](atomic-swap.mdx) uses `incr_allow` to transfer token on behalf of the user
...
This interface can be found in the [SDK]. It extends the common [token interface](tokens.mdx).
