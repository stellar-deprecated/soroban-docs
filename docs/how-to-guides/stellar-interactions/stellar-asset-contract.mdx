...
The Stellar Asset Contract is in early development, has not been audited, and is intended for use in development and testing only at this stage. Report issues [here](https://github.com/stellar/rs-soroban-env/issues/new/choose).
...
The SAC implements the [token interface](../../reference/interfaces/token-interface), which is similar to the widely used ERC-20 token standard. This should make it easier for existing smart contract developers to get started on Stellar.
...
See the [authorization overview](../../learn/contract-development/authorization.mdx) and [auth example](../development-basics/auth.mdx) for general information about authorization in Soroban.

The token contract contains three kinds of operations that follow the token [interface](../../reference/interfaces/token-interface#code):
...
- [Timelock](../financial-transactions/timelock.mdx) and [single offer](../financial-transactions/single-offer-sale.mdx) move token via `xfer` to and from the contract
- [Atomic swap](../financial-transactions/atomic-swap.mdx) uses `incr_allow` to transfer token on behalf of the user
...
This interface can be found in the [SDK]. It extends the common [token interface](../financial-transactions/tokens.mdx).
