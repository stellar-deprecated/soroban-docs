...
### Compatibility with Classic Stellar Assets
...
At that point, the asset can make use of all the features of the Stellar Asset
Contract that are highlighted in this example (allowance, mint, burn, etc.),
while still maintaing the high-quality asset issuer features included with
Classic Stellar.

Additionally, all that comes with **no contract writing required**. Any asset
...
- The `test()` function makes use of a variety of the built-in token functions
  to test the "predictable" way an asset might be interacted with by a user, as
  well as an administrator.
- The `test_burn()` function ensures a `burn()` invocation decreases a user's
  balance, and that a `burn_from()` invocation decreases a user's balance as
  well as consuming another user's allowance of that balance.
- The `transfer_insufficient_balance()` function ensures a `transfer()`
  invocation panics when the `from` user doesn't have the balance to cover it.
- The `transfer_receive_deauthorized()` function ensures a user who is
  specifically de-authorized to hold the token cannot be the beneficiary of a
  `transfer()` invocation.
- The `transfer_spend_deauthorized()` function ensures a user with a token
  balance, who is subsequently de-authorized cannot be the source of a
  `transfer()` invocation.
- The `transfer_from_insufficient_allowance()` function ensures a user with an
  existing allowance for someone else's balance cannot make a `transfer()`
  greater than that allowance.
- The `initialize_already_initialized()` function checks that the contract
  cannot have it's `initialize()` function invoked a second time.
- The `decimal_is_over_max()` function tests that invoking `initialize()` with
  too high of a decimal precision will not succeed.
