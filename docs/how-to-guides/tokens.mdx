...
We could discuss how this standardized interface is useful to let wallets,
users, devs, etc. have a predictablew way of interacting with tokens, but the
behavior behind the interface functions could be Customized for unique needs.
I'm thinking like:

- if it's an NFT token, and each `transfer` function pays a small royalty to the
  original artist
- if it's a tightly controlled asset and the admin has to authenticate every
  transfer in addition to the invoker

I don't think this example contract _actually changes_ any of the default
behavior, but we could point out how it might be done?
...
Maybe this is the place we include descriptions of what `allowance`, `spender`,
`transfer_from`, etc. means. For anyone without an EVM background, they're
looking for this information. Is this the right place for it?
