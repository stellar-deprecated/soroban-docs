...
Due to the fact that a node with diagnostic events enabled will be excuting code paths that diverge from a regular node, we highly encourage only using this feature on watcher node (nodes where `NODE_IS_VALIDATOR=false` is set).
