...
[custom types example]: ../examples/custom-types.mdx
:::
...
```json
{"object":{"map":[{"key":{"symbol":"count"},"val":{"u32":0}},{"key":{"symbol":"last_incr"},"val":{"u32":0}}]}}
```

## Structs (with Unamed Fields)

Structs with unamed fields are stored on ledger as a vector of values, and are
