...
[dependencies]
soroban-sdk = { version = "0.8.1" }
num-integer = { version = "0.1.45", default-features = false, features = ["i128"] }

[dev_dependencies]
soroban-sdk = { version = "0.8.1", features = ["testutils"] }
...
pub fn create_contract(e: &Env, token_wasm_hash: &BytesN<32>, token: &BytesN<32>) -> BytesN<32> {
    let mut salt = Bytes::new(e);
    salt.append(&token.clone().into());
...
It's important to note that the vault contract id will need to change formats for the call to work. We can convert the vault contract id to the correct format by leveraging the [`js-soroban-client`](https://stellar.github.io/js-soroban-client/index.html).
