...
#[contractimpl]
impl SimpleContract {
    ///Example contract for returning Addresses or ids.
    pub fn return_address(env: Env, user: Address) -> Address {
        let current_contract_id = env.current_contract_id();
...
Additionally, Soroban's emphasis on simplicity and safety contributes to its differences in address-related functionality. The use of the `Address` object in Soroban provides a type-safe representation of addresses and allows for easier handling and validation. This helps prevent common programming errors and enhances code security.

The differences in Soroban's address-related functionality, such as retrieving the current contract address and identifier through the `Env` object, provide a more explicit and structured approach to contract development. These differences aim to improve clarity, maintainability, and security in Soroban smart contracts.
...
- `Env::get_ledger_timestamp(self.env())`: Returns a unix timestamp for when the most recent ledger was closed.
- Used as `env.ledger().timestamp()`
- `Env::get_ledger_sequence(self.env())`: Returns the sequence number of the most recently closed ledger.
...
In a [previous example](solidity-and-rust-advanced-concepts#traits), we demonstrated how the `Ownable` trait is implemented for the `OwnableContract` struct. This allows instances of `OwnableContract` to utilize the `is_owner` method defined in the `Ownable` trait.
...
Soroban's approach to authorization in this example offers several advantages over Solidity's model of ERC20 approve and similarby eliminating the need for separate approval management, such as the ERC20 approve method. Instead, authorization checks are directly incorporated into any Soroban function. This simplifies the contract codebase and reduces the complexity associated with managing separate approval states.

Soroban authorization provides Contract-level Authorization, Account Abstraction Functionality, and more advanced Authorization checks. To learn more about these advantages, visit the [Authorization section](../../learn/authorization) of the documentation.

## Summary

Overall the Soroban equivalents of Solidity concepts are very similar. The main difference is that Soroban uses `env` instead of `msg` to access information about the entire contract execution environment, including the state of the contract, addresses involved, and more. Authorization is also handled differently in Soroban, as it is built into the core functionality of the SDK and is more robust than relying on smart contract code alone.
