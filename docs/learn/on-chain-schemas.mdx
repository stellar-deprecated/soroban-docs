---
sidebar_position: 3
title: "On-Chain Schemas: Soroban's secret sauce"
sidebar_label: On-Chain Schemas
---

When you compile a contract created with [soroban-sdk](../reference/sdks/rust.mdx), the Wasm file ends up with a [custom section](https://webassembly.github.io/spec/core/appendix/custom.html) containing a machine-readable description of your contract's interface. That is, its schema. This is similar to [ABIs](https://www.quicknode.com/guides/ethereum-development/smart-contracts/what-is-an-abi/) in Ethereum, except that Soroban will store every single one of them on-chain from day one, and they include the comments from the contract's author.

Today, this schema is formatted using [XDR](https://developers.stellar.org/docs/encyclopedia/xdr), but this [may change](https://github.com/stellar/rs-soroban-sdk/issues/683) down the road.

The important part is that tooling can fetch this schema to make your life easier. [Soroban CLI](../reference/soroban-cli.mdx) is the first tool to do so. Specifically, one subcommand:
...
A unique CLI for each smart contract. Constructed on-the-fly, right from the on-chain schema. Including the author's comments. An _implicit CLI_.
...
```text
$ alias publish="soroban contract invoke --id 6808e8e2cce73cfca472a8197080938b320d72e7b696e34eb52ebf17070f4ee --"
$ publish --help
...
Like any other CLI, you can also get help for any of these subcommands using something like `publish deploy --help`. Soroban CLI again fetches the on-chain schema, this time using it to generate a full list of all arguments to the function, and even generates examples. And just to be totally clear, you don't need to alias these commands. This would work, too:
...
We love that Soroban will have all contract schemas available on-chain right from day one. No secondary API calls to external services, no secondary API token management, no signing in or creating an account anywhere else, and near-perfect reliability. It's a game-changer within the blockchain space.
