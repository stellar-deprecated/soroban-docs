---
sidebar_position: 3
title: "Smart Contract Deployment with Solidity, Rust, and Soroban"
...
# Smart Contract Deployment with Solidity, Rust, and Soroban

In this tutorial, we will discover the similarities in smart contract deployment by examing workflows with Hardhat and Soroban. We will dive into the intricacies of each framework, learn to write secure and efficient smart contract code, and harness the power of Rust and Soroban to create customized contract logic.
...
Soroban is a Rust-based framework tailored for developing smart contracts on the Stellar network. It offers a suite of tools to facilitate the building, testing, and deployment of smart contracts. Designed as a lightweight framework, Soroban allows developers to write smart contracts in Rust and deploy them on the Stellar network.

### Hardhat

Hardhat serves as a development environment for compiling, deploying, testing, and debugging Ethereum software. It assists developers in managing and automating recurring tasks inherent to building smart contracts and dApps while seamlessly introducing more functionality around this workflow.

### Similarities

Both Soroban and Hardhat are frameworks that empower developers to build, test, and deploy smart contracts. They provide an array of tools for constructing, testing, and deploying smart contracts and enable developers to write smart contracts and deploy them on their respective virtual machines.

### Differences

Soroban emphasizes its lightweight design, enabling developers to write smart contracts in Rust and deploy them on the Stellar network. On the other hand, Hardhat is crafted as a comprehensive development environment for the Ethereum Virtual Machine.

## Hardhat vs Soroban SDKs

Hardhat offers a streamlined workflow for deploying smart contracts on the Ethereum Virtual Machine. In this section, we will discuss key components such as `ethers.js`, `scripts`, and `testing`.

Futhermore, we will delve into the [Soroban SDKs](../category/sdks) and examine their parallels with Hardhat components in terms of smart contract development and deployment processes.
...
Soroban offers a comparable library, [`js-soroban-client`](../reference/sdks/js), that enables seamless interaction with the Soroban network. This library supplies a comprehensive networking layer API for `soroban-rpc` methods, simplifying the process of building and signing transactions. Additionally, `js-soroban-client` streamlines communication with `soroban-rpc` instances and supports submitting transactions or querying network state with ease.

### Scripts

Hardhat scripts streamline the automation of routine tasks, such as deploying and managing smart contracts. Developers can create these scripts using either JavaScript or TypeScript, catering to their preferred programming style. They are stored in the `scripts` directory and can be executed using the `npx hardhat run` command.
...
In summary, while Hardhat provides an excellent environment for deploying smart contracts on the EVM, Soroban's Rust-based framework offers significant advantages in terms of safety and performance, making it an ideal choice for building secure and efficient smart contracts.

## Developing Smart Contracts with Rust and Soroban

### Introduction

Now that we've examined the deployment workflow with Hardhat, let's explore developing smart contracts with Rust and Soroban. The key advantage of using Soroban is its ability to leverage Rust's safety features and performance, making it an excellent choice for developing secure and efficient smart contracts.
...
Lets break down what happens in the `lib.rs` file above and find how "yield" is generated from our vault contract
...
Nice! We deposited tokens to the vault and recieved shares in return. Now, let's see what happens when we withdraw tokens from the vault.
...
> _Note_ : In the withdrawal function, you'll notice that the transfer amount is defined as `&(&amount / &100)`. This is a simple yield calculation that assumes the yield to be 1% of the amount being withdrawn. However, it's important to note that this is a very simplistic approach and is not suitable for production-grade systems. The purpose of this calculation is to demonstrate how yield can be generated from a vault contract. In reality, yield calculations are more complex and involve various factors such as market conditions, risk management, and fees.
...
```bash
soroban contract deploy \
    --wasm soroban_vault_contract.wasm \
    --source <SECRET_KEY> \
    --rpc-url https://rpc-futurenet.stellar.org:443 \
    --network-passphrase 'Test SDF Future Network ; October 2022'
```
