...
Let's start by writing a simple smart contract that will allow us to deposit funds into a vault and recieve shares. We'll use this contract as a starting point for using the Soroban SDK to develop Defi applications, and demonstrate how to utilize the soroban-cli tool for interacting with the contract.
