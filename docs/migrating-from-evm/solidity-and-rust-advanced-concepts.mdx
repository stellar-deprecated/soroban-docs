...
then navigate to the `soroban-examples/contracts` directory and run the following command:

```zsh
cargo build --target wasm32-unknown-unknown --release
```

This will build the `soroban_token_contract.wasm` file which you will need to import into your project. The `soroban_token_contract.wasm` file is located in the `soroban-examples/contracts/target/wasm32-unknown-unknown/release` directory.
...
use num_integer::Roots; use soroban_sdk::{contractimpl, Address, BytesN, ConversionError, Env, RawVal, TryFromVal}; use token::create_contract;

pub struct Vault;

#[contractimpl] impl Vault { fn mint(e: &Env, to: Address, shares: i128) { let total_shares = e.storage().get_unchecked(&0u32).unwrap(); e.storage().set(&0u32, &(total_shares + shares)); let balance = e.storage().get_unchecked(&to).unwrap_or(0); e.storage().set(&to, &(balance + shares)); }

    fn burn(e: &Env, from: Address, shares: i128) {
        let balance = e.storage().get_unchecked(&from).unwrap_or(0);
        assert!(balance >= shares, "Vault: insufficient balance");
        e.storage().set(&from, &(balance - shares));
        let total_shares = e.storage().get_unchecked(&0u32).unwrap();
        e.storage().set(&0u32, &(total_shares - shares));
    }

    pub fn deposit(e: Env, amount: i128) {
        let token_balance = token::Client::new(&e, &1u32).balance(&e.current_contract_address());
        let total_shares = e.storage().get_unchecked(&0u32).unwrap();
        let shares = if total_shares == 0 {
            amount
        } else {
            amount * total_shares / token_balance
        };
        Self::mint(&e, e.sender(), shares);
        token::Client::new(&e, &1u32)
            .transfer_from(&e.sender(), &e.current_contract_address(), amount);
    }

    pub fn withdraw(e: Env, shares: i128) {
        let token_balance = token::Client::new(&e, &1u32).balance(&e.current_contract_address());
        let total_shares = e.storage().get_unchecked(&0u32).unwrap();
        let amount = shares * token_balance / total_shares;
        Self::burn(&e, e.sender(), shares);
        token::Client::new(&e, &1u32).transfer(&e.sender(), amount);
    }
...
- `src/token.rs` - This is where we will write our token logic.
...
pub trait VaultTrait {
    ...
    fn withdraw(e: Env, to: Address, share_amount: i128, amount: i128) -> i128;
...
```rust
#![allow(unused)] use soroban_sdk::{Bytes, BytesN, Env};

soroban_sdk::contractimport!(file = "./soroban_token_contract.wasm");

pub fn create_contract(e: &Env, token_wasm_hash: &BytesN<32>, token_a: &BytesN<32>) -> BytesN<32> { let mut salt = Bytes::new(e); salt.append(&token_a.clone().into());
