...
5. [Vault Contract Deployment and Interaction](#vault-contract-deployment-and-interaction)
...
impl VaultTrait for Vault {
    ...
    fn withdraw(e: Env, to: Address, amount: i128) -> i128 {
        ...
        let token_client = token::Client::new(&e, &get_token(&e));
        token_client.xfer(&e.current_contract_address(), &to, &(&amount/&100));
        ...
        if total_shares < amount {
            panic!("not enought shares to complete withdrawal");
        }
...
fn test() {
    ...
    assert_eq!(token.balance(&user), 1000 + yield_amount);
    assert_eq!(token_share.balance(&user), 0);
...
- `transfer_token` is called to transfer tokens from the vault contract to the withdrawer.
- `put_reserve` stores the current token balance in a reserved location.
...
> _Note_ : The walkthrough below requires two futurenet accounts. To create and fund a new account, you can use the [futurenet laboratory](https://laboratory.stellar.org/#account-creator?network=futurenet).
...
soroban contract deploy \
    --wasm soroban_token_contract.wasm \
    --source <FIRST_ACCOUNT_SECRET_KEY> \
    --rpc-url https://rpc-futurenet.stellar.org:443 \
    --network-passphrase 'Test SDF Future Network ; October 2022'
```
...
soroban contract invoke \
    ...
    -- \
    initialize \
    --admin <FIRST_ACCOUNT_ADDRESS> \
    --decimal 18 \
    --name <TOKEN_NAME> \
    --symbol <TOKEN_SYMBOL>
```

After recieving a SUCCESS message, we can mint some tokens. We can do this by running the `mint.sh` script.
...
soroban contract invoke \
    ...
    mint \
    --admin <FIRST_ACCOUNT_ADDRESS> \
    --to <FIRST_OR_SECOND_ACCOUNT_ADDRESS> \
    --amount 100
```

Be sure to mint some tokens to your second account as well. This will be used for "funding" the vault contract.
...
soroban contract invoke \
    ...
    -- \
        balance \
    --id <FIRST_OR_SECOND_ACCOUNT_ADDRESS> 
```
...
After recieving a SUCCESS message, we can deposit some tokens.
Let's start with a deposit from our second account. We can do this by running the `deposit.sh` script. 

```bash
soroban contract invoke \
    --wasm soroban_vault_contract.wasm \
    --id <VAULT_CONTRACT_ID> \
    --source <SECOND_ACCOUNT_ASECRET_KEY> \
    --rpc-url https://rpc-futurenet.stellar.org:443 \
    --network-passphrase 'Test SDF Future Network ; October 2022' \
    -- \
    deposit \
    --to <SECOND_ACCOUNT_ADDRESS> \
    --amount 100
```

And then again from our first account. We can do this by running the `deposit.sh` script. With the `--source` and `--to` flags switched.

```bash
soroban contract invoke \
    --wasm soroban_vault_contract.wasm \
    --id <VAULT_CONTRACT_ID> \
    --source <FIRST_ACCOUNT_ASECRET_KEY> \
    --rpc-url https://rpc-futurenet.stellar.org:443 \
    --network-passphrase 'Test SDF Future Network ; October 2022' \
    -- \
    deposit \
    --to <FIRST_ACCOUNT_ADDRESS> \
    --amount 100
```
...
After recieving a SUCCESS message, we should check our account balance again. We can do this by running the `balance.sh` script.
