...
contract DataTypesExample {
    ...
    // Bytes (fixed-size and dynamic-size)
    bytes2 public fixedBytes = 0x1234;
    bytes32 public dynamicBytes = "Hello, Solidity!";
...
We will explore some of these constructs in more detail in the next article, [Advanced Solidity Concepts](solidity-and-rust-advanced-concepts#advanced-solidity-concepts).
...
The [Soroban Rust SDK](https://docs.rs/soroban-sdk/0.7.0/soroban_sdk/index.html) supports several [built-in data types](../learn/built-in-types), such as:
...
let symbol = Symbol::short("SampleSymbol");
...
These concepts are essential for understanding and writing efficient Rust code and will help you along your Soroban developer journey.
...
Macros in Rust are powerful tools that allow you to define reusable chunks of code. They are defined with the `macro_rules!` keyword and can be invoked with a `!` after the macro's name. Macros employ pattern matching to generate code at compile time.
...
Traits in Rust define a shared set of behaviors for different types. They can be thought of as interfaces in other languages. Traits are defined with the `trait` keyword, and their methods can be implemented for different types using the `impl` keyword.
...
During your Soroban developer journey, you will frequently encounter the attribute macro [`#[contractimpl]`](https://docs.rs/soroban-sdk/0.7.0/soroban_sdk/attr.contractimpl.html) which exports publicly accessible functions to the Soroban environment.
...
Let's break down the [layout of the code](https://docs.soliditylang.org/en/v0.8.19/layout-of-source-files.html#spdx-license-identifier) line by line:
...
Up to this point, we've covered the basics of writing, deploying to a sandbox VM, and interacting with a simple smart contract using Solidity. In the following section, we will extend our knowledge by learning how to achieve the same outcomes using Rust.
...
This code is an implementation of a smart contract written in Rust using the `Soroban SDK`, a Rust-based smart contract development toolkit developed by the [Stellar Development Foundation (SDF)](https://stellar.org/foundation) . The `Soroban SDK` is a powerful Rust library that provides a set of tools for writing smart contracts that run on the [Soroban](https://soroban.stellar.org/) WebAssembly runtime.
...
This is a Rust attribute that tells the Rust compiler not to link the [Rust standard ibrary](https://doc.rust-lang.org/std/). Since Soroban is a [WebAssembly](https://webassembly.org/) runtime, it doesn't have access to the Rust standard library, so this attribute is needed.
...
Now that we have written our smart contract, it's time to explore how we can interact with it using the [Soroban-CLI](https://soroban.stellar.org/docs/reference/soroban-cli). This powerful command-line tool allows us to interact with the Soroban runtime from a local machine, providing us with an efficient and flexible way to manage our smart contract.
...
:::info For a detailed explanation of the `Cargo.toml` configuration used in this tutorial, check out the [Hello World Example](../getting-started/hello-world#import-soroban-sdk-and-features).
