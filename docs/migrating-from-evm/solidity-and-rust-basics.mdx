...
Macros in Rust are powerful tools that allow you to define reusable chunks of code. They are defined with the `macro_rules!` keyword and can be invoked with a `!` after the macro's name. Macros employ [pattern matching](https://doc.rust-lang.org/book/ch18-00-patterns.html) to generate code at compile time.
...
This is a Rust attribute that tells the Rust compiler not to link the [Rust standard ibrary](https://doc.rust-lang.org/std/). Since the Soroban Rust SDK uses its own set of logic, it doesn't need to utilize to the Rust standard library, so this attribute is needed.
...
And that's it! You've learned how to write and interact with simple smart contracts in Solidity and Rust. In the upcoming sections, we'll learn about advanced smart contract concepts, the similarites and diffrences between Solidity and Rust, and how to develop and deploy smart contracts with Soroban.
