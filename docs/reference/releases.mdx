...
- XDR for upgradeable config entries.
- Fix U/I128 and U/I256 number representation for consistent sorting.
- Add contract metadata.
- Remove using "+"" if dirty.
- Update to pick up config settings for contract data.

See https://github.com/stellar/rs-stellar-xdr/releases v0.0.16 for more details.

#### Soroban Environment

- Implement the Arbitrary fuzzing trait for contract value-types.
- Introduce temporary storage in Soroban host.
- Remove unnecessary trait bounds on MeteredVec and MeteredOrdMap.
- Remove admin argument from the token admin interface.
- Token rename.
- Make all the functions starting with "__" reserved.
- Add Comparison fixes.
- Add Token metadata.
- Add soroban_env_host::host::LegacyEpoch.
- Adapt {ui}{128,256} to new xdr definitions; add {ui}256 host functions.
- Add a host function to update the contract Wasm executable.
- Decouple CostModel inputs from iterations; fit and extract model params.
- Don't allow invalid chars in Symbol objects.
- Update rust-version.
- Verify sig order.
- Update host corresponding to fee-related XDR changes.
- Undo pub(crate)ing of fn in host.
- Use network configs for metering.
- Allow re-registering over a contract.
- Fix event name.
- Allow getting top-level authorizations in enforced mode.
- Fix unsoundness in RawVal::get_tag.
- Fix comparison of RawVals of differing types and objectness.
- Expose a list of all auths.
- Remove redundant clone calls.
- Initial configuration of cargo-deny.
- Move to syn2.
...
- Move AuthorizationContext into SDK and add address method.
- Move auth testutils into sdk.
- Rename Client::contract to Client::address.
- Hide warnings about snake case in generated client.
- Return Address when deploy not BytesN(32).
- Remove Address::account_id facility
- Replace BytesN(32) for contract IDs with Address.
- Remove temp storage.
- Remove dependence on auth when registering stellar asset contract in tests.
- Improve docs for mock auth fns.
- Use BytesN(32) for the Salt.
- Introduce temporary storage support in SDK.
- Fix broken link.
- Remove 'admin' param from the SDK token spec.
- Rename token functions.
- bump xdr.
- Set a random hidden source account for test Envs.
- Move the generated token client and token spec into the sdk.
- Remove soroban-token-spec.
- Remove superfluous env parameters.
- Add macro for code metadata.
- Add utils for metadata and token metadata.
- Add conversions from Address to ScAddress.
- Update rust-version.
- Add a function to update current contract's executable with provided Wasm.
- Hide Env.host() in docs.
- Bump env and fix verify order.
- Bump env and sdk for multi-invoke change.
- Make crate path configurable in contractimpl.
- Update soroban-env.
- Update soroban-env, stellar-xdr.
- Default to enforced auth in tests.
- Update rust-version.
- Update soroban-env.
- Add granular mocking of auth in tests.
- initial version of cargo-deny.
