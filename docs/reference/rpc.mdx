...
This example helm chart usage accepts all defaults currently defined on [values.yml](https://github.com/stellar/helm-charts/blob/main/charts/soroban-rpc/values.yaml) which defines network configuration specific to `futurenet`. If you want to change that default behavior, use a different network config, etc., make your own copy of the soroban-rpc helm chart [values.yml](https://github.com/stellar/helm-charts/blob/main/charts/soroban-rpc/values.yaml) locally, review and update all settings under `sorobanRpc.sorobanRpcConfig` and `sorobanRpc.coreConfig` in that file, and then pass that file to `helm install`:
