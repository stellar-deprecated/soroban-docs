...
<head>
  ...
  <link
    rel="canonical"
    href="https://soroban.stellar.org/docs/soroban-internals/interacting-with-contracts"
  />
</head>
...
As mentioned in the [persisting data](../soroban-internals/persisting-data.mdx) section, a contract can only load or store `CONTRACT_DATA` entries that are declared in a _footprint_ associated with its invocation.
