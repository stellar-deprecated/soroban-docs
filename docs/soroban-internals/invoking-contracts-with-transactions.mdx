...
<head>
  ...
  <link
    rel="canonical"
    href="https://soroban.stellar.org/docs/soroban-internals/invoking-contracts-with-transactions"
  />
</head>
...
- Deploy new contracts using the uploaded Wasm or built-in implementations (this currently includes only the [token contract](../tutorials/stellar-asset-contract.mdx)).
...
Soroban's [authorization framework](../soroban-internals/authorization.mdx)
...
[simulate-transaction-doc]: ../soroban-internals/interacting-with-contracts.mdx#authorization
...
[Soroban resource fee](../soroban-internals/fees-and-metering.mdx).
...
`refundableFee` is to use the [`simulateTransaction` mechanism](../soroban-internals/interacting-with-contracts.mdx#transaction-simulation).
