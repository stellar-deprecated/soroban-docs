...
The SAC implements the SEP-41 [token interface][token-interface], which is similar to the
...
defined by the SEP-41 [token interface][token-interface]. But the Stellar
...
```rust
let client = token::StellarAssetInterface::new(&env, &token_id);
