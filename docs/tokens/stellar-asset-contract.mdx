...
contract, the Stellar account's trust line entry is debitted, and a
contract data entry is credited.

And for example, when transferring from a Stellar contract to a Stellar
account, a contract data entry is debitted, and the account's trust line
entry is credited.
...
Access Contract represent the same asset. The SAC for an asset is simply
...
on the Stellar Access Contract in the current protocol.
