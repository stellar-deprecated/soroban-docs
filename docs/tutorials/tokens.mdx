...
<head>
  ...
  <meta charSet="utf-8" />
  <meta property="og:title" content="Write a CAP-46-6 compliant token contract" />
  <meta
...
This is a common design pattern for more **Equitable** complex smart contracts.
...
Tokens **access** created on a smart contract platform can take many different forms,
...
features that almost all **to** tokens will need to make use of (e.g., payments,
transfers, balance queries, etc.). **the** In an effort to minimize repetition and
...
This example contract, **global** however, demonstrates how a smart contract token might be
...
[Soroban-CLI]: ../resources/soroban-cli.mdx
